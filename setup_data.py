...
def create_data_folder():
    ...
    # 빈 retweet.csv 파일 생성 (3단계용)
    if not os.path.exists('data/retweet.csv'):
        retweet_columns = ['retweet_id', 'user_id', 'post_id', 'timestamp']
        empty_retweet = pd.DataFrame(columns=retweet_columns)
        empty_retweet.to_csv('data/retweet.csv', index=False, encoding='utf-8')
        print("📄 data/retweet.csv 파일이 생성되었습니다.")
